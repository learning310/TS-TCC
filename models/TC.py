...
class TC(nn.Module):
    ...
    def forward(self, features_aug1, features_aug2):
        ...
        batch = z_aug1.shape[0]  # features are (batch_size, #channels, seq_len)
        seq_len = z_aug1.shape[2]
        z_aug1 = z_aug1.transpose(1, 2)
        z_aug2 = z_aug2.transpose(1, 2)

        nce = 0  # average over timestep and batch
        # 1. context vector by aug1
        t_samples = torch.randint(seq_len - self.timestep, size=(1,)).long().to(self.device)  # 生成开始的时刻t
        forward_seq = z_aug1[:, :t_samples + 1, :]
        c_t = self.seq_transformer(forward_seq)

        # 2. log-bilinear model
        # 2.1 pred by context vector 
